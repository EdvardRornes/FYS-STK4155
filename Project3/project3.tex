...
\begin{align}
	...
	&= \prod_{j=0}^{n-k-1} \left(\sigma'_{h}(z_{z-j})W_{hh}\right).
\end{align}
...
\begin{subequations}
\begin{align}
	\frac{\partial L}{\partial W_{hh}} &= \sum_{n=1}^{N}\sum_{k=1}^{n}\frac{\partial L}{\partial h_{n}}  \left[ \prod_{j=n-k-1}^{0} \sigma'_{h}(z_{n-j})W_{hh}\right] \delta^{k}_{hh}, \\
	\frac{\partial L}{\partial W_{hx}} &= \sum_{n=1}^{N}\sum_{k=1}^{n}\frac{\partial L}{\partial h_{n}}  \left[ \prod_{j=n-k-1}^{0} \sigma'_{h}(z_{n-j})W_{hh}\right] \delta^{k}_{hx}, \\
	\frac{\partial L}{\partial b_{h}} &= \sum_{n=1}^{N}\sum_{k=1}^{n}\frac{\partial L}{\partial h_{n}}  \left[ \prod_{j=n-k-1}^{0} \sigma'_{h}(z_{n-j})W_{hh}\right] \sigma'_{h}(z_k).
\end{align}
\end{subequations}
...
\end{subequations}
\begin{align}
	\frac{\partial L}{\partial W_{yh}} &= \frac{\partial L}{\partial h_{N}}  \left[ \prod_{j=N-k-1}^{0} \sigma'_{h}(z_{n-j})W_{hh}\right] \sigma'_{\text{out}}(z_N)h_N, \\
	\frac{\partial L}{\partial b_{h}} &= \frac{\partial L}{\partial h_{N}}  \left[ \prod_{j=N-k-1}^{0} \sigma'_{h}(z_{n-j})W_{hh}\right] \sigma'_{\text{out}}(z_N).
\end{align}
...
\end{itemize}
where $N$ is the number of epochs. Due to the program being incredibly slow, we only managed to finish half the parameter scan with $N=100$. Similarly, due to spending much time implementing our own RNN, we only had time to perform the parameter scans with the following parameters:
\begin{itemize}
	\item $N\in\{\}$
	\item $\phi\in\{\}$
	\item $\lambda\in\{\}$
