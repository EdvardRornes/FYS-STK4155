...
In this project, we implement a Feedforward Neural Network (FFNN) to analyze the Franke function. The FFNN is structured with an input layer, one or more hidden layers, and an output layer, with each layer utilizing an activation function. The architecture is defined by specifying the input size, the number of neurons in hidden layers, and the output size. 

The weights and biases are initialized using random values scaled by the number of input neurons, which aids in faster convergence during training. The network supports several activation functions, including ReLU, Sigmoid, and Leaky ReLU. The activation functions are chosen based on their performance in the forward pass, where the input data is transformed through the layers of the network.
