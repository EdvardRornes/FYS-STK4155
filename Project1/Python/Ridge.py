...
plt.figure(figsize=(10, 6))
plt.title(rf"MSE deg {deg_analysis} (Franke).")
line = plt.plot(np.log10(lmbdas), MSE_train_array, label="MSE train", lw=2.5)
color = line[0].get_color()
plt.plot(np.log10(lmbdas), MSE_test_array, "--", color=color, label="MSE test", lw=2.5)
...
plt.figure(figsize=(10, 6))
plt.title(rf"$R^2$ with deg {deg_analysis} (Franke).")
line = plt.plot(np.log10(lmbdas), R2_train_array,label=r"$R^2$ train", lw=2.5)
color = line[0].get_color()
plt.plot(np.log10(lmbdas), R2_test_array, "--", color=color, label=r"$R^2$ test", lw=2.5)
plt.xlabel(r"$\log_{10}(\lambda)$")
plt.ylabel(r"$R^2$")
plt.xlim(lambda_exp_start, lambda_exp_stop)
plt.legend()
