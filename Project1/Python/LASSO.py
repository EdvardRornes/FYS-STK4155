...
for l in lambdas:
    for deg in range(deg_max):
        ...
        beta[deg], MSE_train[deg], MSE_test[deg], R2_train[deg], R2_test[deg] = LASSO_fit(X_train, X_test, z_train, z_test)
...
plt.xlim(1, deg_max)
plt.title(r'LASSO $\beta$ coefficients')
plt.legend()
plt.grid(True)
plt.savefig(f'Figures/LASSO-beta-degree.pdf')
plt.show()
'''