...
Cross validation is particularly advantageous for its ability to reduce the variance of performance estimates by using different portions of the data for both training and validation. In our analysis, we used 10-fold cross validation to evaluate the stability and predictive power of each regression method. This method gave us a clearer picture of how each model generalizes, helping us tune the hyperparameters, such as the regularization parameter $\lambda$ for Ridge and LASSO.
...
The MSE of all the three regression methods OLS, Ridge and LASSO with \(\lambda=10^{-9}\) and standard scaling is shown in \ref{fig:MSE_GrandCan_StandardScaling_17} and \ref{fig:MSE_MountEve_StandardScaling_17} using cross validation, for Grand Canyon and Mount Everest, respectively. As mentioned prior, due to computational purposes we have chosen to work with only a small subsection of the full data. In particular we only work with $N=31$ points as the program already begins to slow down for large \(N\) and \(p\).
...
Both figures show the cross validated Ridge and OLS following each other until they both increase, with greater error bars. OLS seems to increase slightly faster for large \(p\). LASSO on the other hand, slowly decreases, seemingly reaching a constant value as \(p\) increases, above Ridge and OLS. Visually, the lowest MSE is found with Ridge and a polynomial degree in the approximate range \(p\in[20, 25]\) for both terrains. 
...
\begin{figure}[H]
	\centering
	\includegraphics[width=\linewidth]{Python/Figures/Terrain/Terrain_CV_Regression_Grand Canyon_StandardScaling_30.pdf}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=\linewidth]{Python/Figures/Terrain/Terrain_CV_Regression_Mount Everest_StandardScaling_30.pdf}
...
Figs. \ref{fig:log_GrandCan_StandardScaling_10} and \ref{fig:log_MountEve_StandardScaling_10} show the MSE and $R^2$ of Ridge and LASSO as a function of \(\lambda\) with \(p=23\) for both Grand Canyon and Mount Everest, respectively. For both Grand Canyon and Mount Everest, Ridge seems to outperform LASSO for all \(\lambda\)'s. For Ridge, the figures suggests a slightly better MSE for lower values of \(\lambda\). 

Note that there are additional contour- and 3D-plots in Appendix \ref{Appendix:Plots}.

\begin{figure}[H]
	\centering
	\includegraphics[width=0.95\linewidth]{Python/Figures/Terrain/log_MSE_R2_Grand Canyon_StandardScaling_23.pdf}
	\caption{MSE and $R^2$ of Grand Canyon terrain data with $p=23$ for Ridge and LASSO as a function of the hyperparameter $\lambda$.}
	\label{fig:log_GrandCan_StandardScaling_10}
\end{figure}

\begin{figure}[H]
	\centering
	\includegraphics[width=0.95\linewidth]{Python/Figures/Terrain/log_MSE_R2_Mount Everest_StandardScaling_23.pdf}
	\caption{MSE of Mount Everest terrain data with $p=23$ for Ridge and LASSO as a function of the hyperparameter $\lambda$.}
	\label{fig:log_MountEve_StandardScaling_10}
\end{figure}

\section{Conclusion}
In conclusion we find that the terrain data is much more complicated than the Franke function, as one would expect, and thus requiring a much larger polynomial degree to optimize the MSE and \(R^2\)-score. OLS and Ridge outperformed LASSO on both the Franke function and terrain data. In fact, OLS and Ridge performed roughly equal throughout all the data sets, but with the latter method taking a slight edge. The current hypothesis for this, following the discussion in section \ref{sec:stats}, is that LASSO performs worse due to the $\beta$ coefficients not following a Laplace distribution. If these coefficients do instead follow a Gaussian distribution and that there is a low correlation between the different events, then this is exactly what one would expect. One positive performance for the LASSO method was its insensitivity to overfitting, as it flattened out when the polynomial degree increased. 

The expected bias-variance trade-off was observed explicitly by using bootstrapping, for the OLS method. For higher (lower) polynomial degrees we clearly see an increase in the variance (bias), which helped us choose which model complexity we should aim for. The cross validation algorithm provided error bars for the MSE, which helped us assess the stability of the three regression methods. Whilst the stability of LASSO remained consistent throughout, Ridge and OLS both had their error bars increase as $p$ increased.
...
We did not spend time making these plots look nice nor fix captions for them but hopefully the titles are explanatory enough. Note that for the predicted versions of the terrain data we have used StandardScaling whilst the actual terrain file is not. $\lambda=10^{-9}$ is used throughout.
\subsection{Franke}
\begin{figure}[H]
	\includegraphics[width=0.4\textwidth]{Python/Figures/Terrain/OLS_franke_Unscaled_3D.pdf}
\end{figure}
\begin{figure}[H]
	\includegraphics[width=0.4\textwidth]{Python/Figures/Terrain/Ridge_franke_Unscaled_3D.pdf}
\end{figure}
\begin{figure}[H]
	\includegraphics[width=0.4\textwidth]{Python/Figures/Terrain/LASSO_franke_Unscaled_3D.pdf}
\end{figure}
\subsection{Grand Canyon}
\begin{figure}[H]
	\includegraphics[width=0.4\textwidth]{Python/Figures/Terrain/Grand Canyon_actual.pdf}
\end{figure}
\begin{figure}[H]
\includegraphics[width=0.4\textwidth]{Python/Figures/Terrain/OLS_Grand Canyon_StandardScaling.pdf}
\end{figure}
\begin{figure}[H]
	\includegraphics[width=0.4\textwidth]{Python/Figures/Terrain/Ridge_Grand Canyon_StandardScaling.pdf}
\end{figure}
\begin{figure}[H]
\includegraphics[width=0.4\textwidth]{Python/Figures/Terrain/LASSO_Grand Canyon_StandardScaling.pdf}
\end{figure}
\begin{figure}[H]
	\includegraphics[width=0.4\textwidth]{Python/Figures/Terrain/Grand Canyon_actual_3D.pdf}
\end{figure}
\begin{figure}[H]
\includegraphics[width=0.4\textwidth]{Python/Figures/Terrain/OLS_Grand Canyon_StandardScaling_3D.pdf}
\end{figure}
\begin{figure}[H]
	\includegraphics[width=0.4\textwidth]{Python/Figures/Terrain/Ridge_Grand Canyon_StandardScaling_3D.pdf}
\end{figure}
\begin{figure}[H]
\includegraphics[width=0.4\textwidth]{Python/Figures/Terrain/LASSO_Grand Canyon_StandardScaling_3D.pdf}
\end{figure}
\subsection{Mount Everest}
\begin{figure}[H]
	\includegraphics[width=0.4\textwidth]{Python/Figures/Terrain/Mount Everest_actual.pdf}
\end{figure}
\begin{figure}[H]
\includegraphics[width=0.4\textwidth]{Python/Figures/Terrain/OLS_Mount Everest_StandardScaling.pdf}
\end{figure}
\begin{figure}[H]
	\includegraphics[width=0.4\textwidth]{Python/Figures/Terrain/Ridge_Mount Everest_StandardScaling.pdf}
\end{figure}
\begin{figure}[H]
\includegraphics[width=0.4\textwidth]{Python/Figures/Terrain/LASSO_Mount Everest_StandardScaling.pdf}
\end{figure}
\begin{figure}[H]
	\includegraphics[width=0.4\textwidth]{Python/Figures/Terrain/Mount Everest_actual_3D.pdf}
\end{figure}
\begin{figure}[H]
\includegraphics[width=0.4\textwidth]{Python/Figures/Terrain/OLS_Mount Everest_StandardScaling_3D.pdf}
\end{figure}
\begin{figure}[H]
	\includegraphics[width=0.4\textwidth]{Python/Figures/Terrain/Ridge_Mount Everest_StandardScaling_3D.pdf}
\end{figure}
\begin{figure}[H]
	\includegraphics[width=0.4\textwidth]{Python/Figures/Terrain/LASSO_Mount Everest_StandardScaling_3D.pdf}
\end{figure}
