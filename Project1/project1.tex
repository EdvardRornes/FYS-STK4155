...
\section{Results}
As an general observation, we found that the LASSO-method performed the worst out of the three methods, while OLS and Ridge performing similarly, however dependent on the choice of \(\lambda\). With an increasing (maximum) polynomial degree the MSE of OLS and Ridge blew up, while LASSO settled horizontally.
All three models had the largest \(\beta\)-coefficients corresponding to an polynomial degree of approximately \(2-3\).

Between the choice of min-max scaling, standard scaling and no scaling, it was found that no scaling gave the best result. Min-max scaling showed little to no impact on both the MSE and the \(R^2\)-score, while standard scaling, in contrast, led to a deterioration in both metrics.

We found that all the three models are quite dependent on the size \(N\) of the sample-data set and on \(p\). Keeping \(p\) fixed and increasing \(N\), we found that the MSE of both the test and the training-data decreased. To better analyzie the performance of the three models we found that with \(N=25\) and \(p=25\).
