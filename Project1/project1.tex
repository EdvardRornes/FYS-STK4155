...
Using the Bootstrap method on the Franke function with OLS regression, $N=25$, $\varepsilon=0.01$ and \(100\) samples, the bias, variance and error is shown in Fig. \ref{fig:bootstrap}. The figure shows a close-to constant variance for polynomial degrees smaller than \(p\sim7\), whereas for \(p\gtrsim 7\) the variance increases steadily. On the other hand, the bias starts out above \(\sim0.1\), gradually decreasing, reaching a minimum for \(p\) in the range \(4-9\), before rising again for \(p\gtrsim 9\). These specific values of \(p\) is dependent on the sample size \(N=25\) and the amount of samples taken by the Bootstrap method. However, the general shape of the error curve in Fig. \ref{fig:bootstrap} is expected independent of these settings. The error is supposed to be proportional to the sum of the variance and the bias, as seen in \eqref{eq:MSE-bias-var}. Visually, Fig. \ref{fig:bootstrap}. confirms this, although the sum is not explicitly given. 
...
Figs. x,y,z, show the MSE and $R^2$ for all three regression methods. Additionally we have plotted the $\lambda$ dependence of Ridge and LASSO for the MSE and $R^2$ with polynomial degree $p=44$ in Figs. x,y. We see that LASSO consistently performs worse across all metrics compared to OLS and Ridge. 

Further in Fig. x we plot the MSE of all regression models as a function of the polynomial degree where he have used $k=10$-fold CV.
%\begin{figure}[ht!]
%	\centering
%	\includegraphics[width=\linewidth]{Python/Figures/Terrain/LASSO_logMSE_Grand Canyon_StandardScaling_44.pdf}
%	\caption{Terrain LASSO MSE as a function of $\lambda$}
%	\label{fig:Terrain_Lasso_MSE_lambda}
%\end{figure}
%\begin{figure}[ht!]
%\centering
%\includegraphics[width=\linewidth]{Python/Figures/Terrain/LASSO_logR2_Grand Canyon_StandardScaling_44.pdf}
%\caption{Terrain LASSO RIDGE as a function of $\lambda$}
%\label{fig:Terrain_Lasso_R2_lambda}
%\end{figure}
