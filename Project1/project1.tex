...
\end{align}
This function maps a surface defined on the interval $x,y\in[0,1]$. To perform an analysis on this function we perform a polynomial fit up to degree $n$ where
\begin{align*}
	\tilde{\bm{z}}&=\beta_{00}\\
	&+\beta_{10}x+\beta_{11}y\\
	&+\beta_{20}x^2+\beta_{21}xy+\beta_{22}y^2\\
	&+...\\
	&+\beta_{n0}x^n+\beta_{n1}x^{n-1}y+...+\beta_{n(n-1)}xy^{n-1}+\beta_{nn}y^n\\
	&\equiv \bm X\bm\beta
\end{align*}
i.e. the elements of the design matrix $(\bm X)_{ij}=x^{i-j}y^{j}$ for $n\geq i\geq j$. 
\section{Results}
