...
\begin{align*}
	\mathbb{E}(y_i)=\mathbb{E}(f(x_i))=\sum_{j=0}^{p-1}X_{ij}\beta_j\equiv\bm X_{i,*}\bm\beta,
\end{align*}
...
\end{align}
This has the added benefit of being able to set certain parameters to be $0$ instead of suppressing them, at the cost of losing analytical expressions for $\bm\beta$ in non-trivial cases \cite{hastie01statisticallearning}.
...
We represent and discuss all the most relevant results for both the Franke function and the terrain data in this section. For additional plots, see Appendix \ref{Appendix:Misc}.
...
The MSE and $R^2$-score for both the train and test data as a function of the polynomial degree are given in Fig. \ref{fig:OLS_mse_degree}. The MSE of the test data exhibits a square-well shape with its lowest point at around \(p=6\), while the MSE of the training data steadily decreases as \(p\) increases. Similar traits are seen for the $R^2$ plot, however mirrored vertically. This is as expected for low $p$ as the Franke function is too complicated to be accurately described by low order polynomials. Once the polynomial degree becomes too high we start to clear signs of overfitting as we see sharp increases (decreases) in the MSE ($R^2$). 
\begin{figure}[ht!]
	\centering
...
To view the dependence on $\lambda$ more explicitly we fixed the degree to \(p=6\) and used 100 logarithmically spaced values of $\lambda$ and plotted the MSE and $R^2$ as a function of $\lambda$ in Fig. \ref{fig:ridge_logMSE_degree}. The chosen value of $p=6$ was considered due to the diminishing returns from increasing \(p\) past this value, as seen in Fig. \ref{fig:ridge_mse_degree}. These plots suggest an optimal value of $\lambda\sim 10^{-6}$ as this is where the MSE ($R^2$) is the lowest (highest) and is thus the value that was used for further analysis.

\begin{figure}[ht!]
	\centering
	\includegraphics[width=\linewidth]{Python/Figures/Ridge/RIDGE_logMSE_R2_Unscaled.pdf}
	\caption{Ridge MSE and $R^2$ with max polynomial degree $6$ as a function of the hyperparameter $\lambda$.}
	\label{fig:ridge_logMSE_degree}
\end{figure}

\subsubsection{LASSO}
At last we look at the results for LASSO. Again the MSE and $R^2$-scores as a function of the polynomial degree are given in Fig. \ref{fig:lasso_mse_degree}. Unlike in the cases of OLS and Ridge, these plots are strikingly constant, showing little to no variation. 
\begin{figure}[ht!]
	\centering
	\includegraphics[width=\linewidth]{Python/Figures/LASSO/LASSO_MSE_R2_Unscaled.pdf}
	\caption{LASSO MSE and $R^2$ as a function of the max polynomial degree with \(p_{\text{max}}=17\). The three chosen values for the hyperparameter $\lambda$ are $10^{-10},10^{-3}$ and $10^{-2}$. The remaining description can be found under Fig. \ref{fig:ridge_mse_degree}.}
...
\begin{figure}[ht!]
	...
	\caption{LASSO MSE and $R^2$ with max polynomial degree $4$ as a function of the hyperparameter $\lambda$.}
...
Both figures show the cross validated Ridge and OLS following each other until OLS increases, jaggedly and with greater error bars, away from Ridge. 
LASSO on the other hand, slowly decreases, seemingly reaching a constant value as \(p\) increases, above Ridge. 
Visually, the lowest MSE is found with Ridge and a polynomial degree in the approximate range \(p\in[]\). 
It can also be seen that the MSE of Mount Everest is much greater for low order polynomials, as is something that could be expected for mountainous terrain. 
However it must be noted that this subsection we are working with does not properly represent the entire mountain, 
but instead a small patch on the bottom left of Fig. \ref{fig:GC_ME_Image_Contour}. As such in the case of the Grand Canyon we do not map the parts 
with large cliffs but instead mostly flatland. Thus one may expect a lower order polynomial regression performs relatively good here.
...
Figs. \ref{fig:log_GrandCan_StandardScaling_35} and \ref{fig:log_MountEve_StandardScaling_35} show the \(R^2\)-score of Ridge and LASSO as a function of \(\lambda\) with \(p=35\) for the Grand Canyon and Mount Everest respectively. The two figures show Ridge giving a higher \(R^2\)-score than LASSO for all \(\lambda\in[10^{-10}, 10^{-2}]\). For Mount Everest, the maximum \(R^2\)-score is located at \(\lambda\sim 10^{-9}\), whilst Grand Canyon gives about the same \(R^2\)-score for \(\lambda\in[10^{-10}, 10^{-6}]\). It must however be noted that these $R^2$ scores are incredibly close to 1, which may be too good to be true. One could imagine that this could be due to our subsection of the terrain is topologically flat compared to the Franke function, and thus causing our fit to be better. However considering the fact that we do not overfit the model until a much higher polynomial degree compared to the Franke function. Thus we believe that this contradicts our expectation and conclude that there must be a mistake somewhere in the code.
...
On the terrain data we still see that OLS and Ridge outperform LASSO, but all regression models perform much better than expected. This is likely due to mistakes in the code which we were unable to detect. As such many of the results in this report should not be taken at face value and instead be subject to scrutiny.
